...
route.post("/", validateEmail, async (req, res) => {
	...
	if (!req.body.password)
		return res.status(400).json({ detail: "Password is a required property" });
	if (!req.body.name)
		return res
			.status(400)
			.json({ detail: "User's name is a required property" });

	if (!req.body.is_supplier && null)
		return res
			.status(400)
...
route.put("/:uuid", validateUUID, validateEmail, async (req, res) => {
	...
	const is_supplier = userData.is_supplier || userResponse.rows[0].is_supplier;
